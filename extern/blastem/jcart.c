...
uint16_t jcart_read_w(uint32_t address, void *context)
{
	...
	uint16_t value = io_data_read(ports, m68k->current_cycle, NULL) << 8;
	value |= io_data_read(ports + 1, m68k->current_cycle, NULL);
	return value;
}
...
uint8_t jcart_read_b(uint32_t address, void *context)
{
	m68k_context *m68k= context;
	io_port *ports = get_ports(m68k);
	return io_data_read(ports + (address & 1), m68k->current_cycle, NULL);
}
