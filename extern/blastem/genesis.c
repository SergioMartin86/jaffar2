...
uint8_t *serialize(void *sys, size_t *size_out)
{
...
{
 genesis_context * gen = context->system;
// if (_curFrameId > 1700 && activated == 0) {handleError("test"); activated = 1;}
	if (vdp_port & 0x2700E0)
	{
	 handleError("Warning  1\n");
//		fatal_error("machine freeze due to write to address %X\n", 0xC00000 | vdp_port);
		return context;
	}

	if (!gen->vdp_unlocked) {
	 handleError("Warning  2\n");
...
//				fatal_error("Machine freeze due to read of Z80 VDP memory window by 68K: %X\n", location | 0xA00000);
		...
		if (location < 0x10100) {
			...
			case 0x0:
				...
				break;
			case 0x1:
				value = io_data_read(gen->io.ports, context->current_cycle, context);
				break;
			case 0x2:
				value = io_data_read(gen->io.ports+1, context->current_cycle, context);
				break;
			case 0x3:
				value = io_data_read(gen->io.ports+2, context->current_cycle, context);
				break;
			case 0x4:
...
void free_genesis(system_header *system)
{
	genesis_context *gen = (genesis_context *)system;
